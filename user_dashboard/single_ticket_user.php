...
<body>
                                     ...
                                     if ($row_cust > 0) {
                                        ...
                                        while ($row=mysqli_fetch_array($cust)) {
                                            ...
                                            echo "<div class='alert alert-danger' role='alert'>
                                              This ticket is closed. You may continue by opening a new ticket or sending us a mail.
                                            </div>";
                                        }else{
                                ...
                                <div class="col">
                                    ...
                                    $id=$_GET['id'];
                                      ...
                                      //   $row_customers=mysqli_fetch_array($customers);
                                        ...
                                        $row_cust=mysqli_fetch_array($cust);
                                    
                                     ?>
                                        <div>
                                            <h3 class="">
                                                <i class="fe fe-user" style="font-size: 25px;"></i>User
                                            </h3>
                                            <p class="" style="font-size: 11px; text-align: right; margin-top: -30px">
                                                <?php echo "$row_cust[date_created]"; ?>
                                            </p>
                                        </div>
                                        <hr class="my-4">
                                        <p class="">
                                            <?php echo "$row_cust[text]"; ?>
                                        </p>
                                        <!-- Reply part -->
                                        <div class="card">
                                            <?php
                                                ...
                                                $user_ticket=mysqli_query($con, "SELECT * FROM `reply_ticket` WHERE ticket_id = '$id' ");
                                        
                                                    $row_user=mysqli_num_rows($user_ticket);
                                                    if ($row_user > 0){

                                                        while ($row=mysqli_fetch_array($user_ticket)) {

                                                     echo "<div class='card-body'>
                                                     <h3 class=''>
                                                <i class='fe fe-user' style='font-size: 25px;'></i>Admin
                                            </h3>".$row['text']."</div> <hr>"; 
    ...
    if (isset($_POST['submit'])) {
      ...
      $sql = mysqli_query($con, "INSERT INTO `reply_ticket`(`id`, `ticket_id`, `user_id`, `text`, `status`, `is_active`, `date`) VALUES (NULL,'$id','$row_user[id]','$message','0','0','$date')");


      if ($sql) {

        // $ticket_update=mysqli_query($con, "UPDATE `support_ticket` SET `status`='$status' WHERE user_id='$id'");
