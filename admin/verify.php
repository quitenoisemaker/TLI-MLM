...
if (isset($_GET['vkey'])) {
	...
	$sql = "SELECT * FROM customers where status ='".$status."' AND vkey= '".$vkey."' LIMIT 1";

	$result = $con->query($sql);

	if($result->num_rows>0){

		  $sql = "UPDATE customers SET status='$verified' WHERE vkey='$vkey' LIMIT 1";

		  if(mysqli_query($con, $sql)) {

		  	$message="
		  		<i class='fa fa-thumbs-up' style='font-size: 100px; color: #000'></i><br><br>
		  	You account has been verified. You may <a href='sign-in' class='text-success'>login</a>";
	...
	}else
		    {
		    	...
		    	$message2="The account invalid or register already";
		    }
}else{
