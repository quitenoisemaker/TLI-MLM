      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-12 col-lg-6">
            <!-- Card -->
            <div class="card">
              <div class="card-header text-center"><h3>Bank details</h3></div>
              <div class="card-body">
                    ...
                    $sql = "SELECT * from  customers where id = $id";
                     ...
                     <!-- Text -->
                        <small class="">
                          <?php
                            $id=$_GET['id'];
                           $sql = "SELECT * FROM payment_proof where user_id =$id";
                            $result = $con->query($sql);
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){
                            $image=$row["img"];
                            echo "
                            <div class='gallery'>
                            <a href='img3/$image'><img src='img3/$image' height='60' width='60'></a>
                            </div>";
                             }
                      }else{
                    ...
                    $sql = "SELECT * from  customers where id = $id";
                          ...
                          if ($result->num_rows >0) {
                            ...
                            while ( $row = $result->fetch_assoc()){
                              if ($row['status']==0) {?>
                                <?php if($row['status']=="" || $row['status']==0){ ?>
                        <a class="btn btn-primary" href="pending?aeid=<?php echo htmlentities($row['id']);?>" onclick="return confirm('Do you really want to Activate user?')">Activate</a>
                        <?php }else {?>
  ...
  <script>
    ...
    if(isset($_GET['aeid'])){
      ...
      echo "<script>alert('User Successfully Activated')</script>";
          ...
          echo "<script>window.open('single_members','_self')</script>";
      }
