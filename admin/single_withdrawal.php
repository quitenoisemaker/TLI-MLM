...
<!-- Copied from https://dashkit.goodthemes.co/account-members.html by Cyotek WebCopy 1.8.0.652, Wednesday, 17 June 2020, 1:15:23 pm -->
  ...
  <body>
    ...
    <div class="main-content">
                      ...
                      $sql = "SELECT * FROM customers where id ='".$id."'";
                        ...
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){
                            $image=$row["img"];
                            echo "<img src='img/$image' class='avatar-img rounded-circle border border-4 border-body' alt=''>";
                             }
                      }
                    }
                  ?>
      ...
      <div class="container-fluid">
                ...
                <div class="row align-items-center">
                  <div class="col">
                    ...
                    <span class="h2 mb-0">
                      ...
                      $get_wallet=mysqli_query($con, "SELECT * FROM `ewallet` WHERE user_id= '$id'");
                      $row_wallet=mysqli_fetch_array($get_wallet);

                      echo "NGR" ." ". number_format($row_wallet['current_balance']);      
                       ?>
                    </span>
                ...
                <div class="row align-items-center">
                  <div class="col">
                    ...
                    <span class="h2 mb-0">
                      <?php
                      $status=0;
                      $get_wallet=mysqli_query($con, "SELECT * FROM `fund_withdrawal` WHERE user_id= '$id' and status ='$status'");
                      $row_wallet=mysqli_fetch_array($get_wallet);

                      echo "NGR" ." ". number_format($row_wallet['amount']);      
                       ?>
        ...
        <!-- Button -->
                <?php
                  $id=$_GET['id'];
                    $sql = "SELECT * from  fund_withdrawal where user_id = $id";
                          $result=$con->query($sql);
                          if ($result->num_rows >0) {
                        ...
                        <a class="btn btn-primary" href="withdraw_pending?aeid=<?php echo htmlentities($row['user_id']);?>" onclick="return confirm('Do you really want to Activate user?')">Approve</a>
                        <?php }else {?>
    ...
    if(isset($_GET['aeid'])){
      ...
      $sql = "UPDATE fund_withdrawal SET status=$status WHERE  id=$aeid";
      $query =$con->query($sql);
      $msg="User Deactivated";
      echo "<script>alert('User Successfully Activated')</script>";

          echo "<script>window.open('single_withdrawal','_self')</script>";
      }
