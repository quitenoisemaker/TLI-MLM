...
<body>
                        ...
                        <div class="table-responsive mb-0" data-list="{&quot;valueNames&quot;: [&quot;goal-project&quot;, &quot;goal-status&quot;, &quot;goal-progress&quot;, &quot;goal-date&quot;]}">
                            <table id="table_id" class="table table-sm table-nowrap card-table">
                                ...
                                <tbody class="list">
                                    <?php
                  $status= 0;
                    $sql1 = "SELECT * from  fund_withdrawal where status = $status";
                    ...
                    { ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="single_withdrawal?id=<?php echo $row1['user_id'];?>"><span class="badge badge-info btn-sm"><i class="fas fa-eye"></i></span></a>
                                        </td>
                                        <td>
                                            <?php if($row['status']=="" || $row['status']==0){ ?>
                                            <a class="badge badge-warning btn-sm" href="withdraw_pending?aeid=<?php echo htmlentities($row1['user_id']);?>" onclick="return confirm('Do you really want to Approve user?')">Activate</a>
                                            <?php }else {?>
  ...
  if($result->num_rows>0){
    while($row=$result->fetch_assoc()){
      ...
      $sql = "UPDATE fund_withdrawal SET status=$status, approved_by=$admin WHERE  user_id=$aeid";
      $query =$con->query($sql);
      echo "<script>alert('User Successfully Approved')</script>";

                echo "<script>window.open('withdraw_pending','_self')</script>";
      }
