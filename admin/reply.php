...
<body>
    ...
    <div class="main-content">
        ...
        <div class="header">
            ...
            <div class="container-fluid">
                <!-- Body -->
                <div class="header-body mt-n5 mt-md-n6 d-flex justify-content-center">
                        ...
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col">
                                    ...
                                    $cust=mysqli_query($con, "SELECT * FROM `support_ticket` WHERE unique_id = '$id' ");
                                    $row_support=mysqli_fetch_array($cust);

                                      $customers=mysqli_query($con, "SELECT * FROM `customers` WHERE id = '$row_support[user_id]' ");
                                        
                                        $row_customers=mysqli_fetch_array($customers);
                                     ...
                                     ?>
                                        <div>
                                            <h5 class=""><i class="fe fe-user" style="font-size: 25px;"></i>
                                                <?php echo "$row_customers[fname]"; ?>
                                            </h5>
                                            <h5 class="" style="font-size: 11px; text-align: right; margin-top: -30px">Created:
                                                <?php echo "$row_support[date_created]"; ?>
                                            </h5>
                                        </div>
                                        <hr class="my-4">
                                        <p class="">
                                            <?php echo "$row_support[text]"; ?>
                                        </p>
                                        ...
                                        $get_ticket=mysqli_query($con, "SELECT * FROM `reply_ticket` WHERE ticket_id='$id' ");

                                        $row_ticket=mysqli_num_rows($get_ticket);

                                                  if ($row_ticket > 0) {
                                    # code...
                                    while ($row=mysqli_fetch_array($get_ticket)) {
    ...
    if (isset($_POST['submit'])) {
      ...
      $get_ticket=mysqli_query($con, "SELECT * FROM `support_ticket` WHERE unique_id='$id' ");

      $row_ticket=mysqli_fetch_array($get_ticket);

      $get_user=mysqli_query($con, "SELECT * FROM `customers` WHERE id='$row_ticket[user_id]' ");
      $row_user=mysqli_fetch_array($get_user);

      $sql = mysqli_query($con, "INSERT INTO `reply_ticket`(`id`, `ticket_id`, `user_id`, `text`, `status`, `is_active`, `date`) VALUES (NULL,'$id','$row_user[id]','$message','1','0','$date')");
      ...
      if ($sql) {
        ...
        $ticket_update=mysqli_query($con, "UPDATE `support_ticket` SET `status`='$status' WHERE unique_id='$id'");

 
        echo "<script>alert('Successful')</script>";
        echo "<script>window.open('pending_ticket','_self')</script>";
      }else{
